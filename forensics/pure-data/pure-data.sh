...
# Determine Pure Data base directory
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PURE_DATA_DIR="${PURE_DATA_DIR:-$SCRIPT_DIR}"
...
check_requirements() {
    ...
    if [[ -d "$PURE_DATA_DIR" ]]; then
        echo -e "✅ Pure Data workspace - Available"
        
        # Check for main tools
        if [[ -x "$PURE_DATA_DIR/tools/android-master-menu.sh" ]]; then
...
launch_pure_data() {
    ...
    if [[ ! -f "$PURE_DATA_DIR/tools/android-master-menu.sh" ]]; then
    ...
    chmod +x "$PURE_DATA_DIR/tools/android-master-menu.sh" 2>/dev/null
    
    # Launch the main menu
    exec "$PURE_DATA_DIR/tools/android-master-menu.sh"
}
